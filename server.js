...
app.get('/delete', function(req, res){
    if(req.query.owner == req.session.userid){
        ...
        return res.status(200).render('info', {message: "Access denied - You don't have the access and deletion right. You are using user: " + req.session.userid + " ."}
...
app.get('/api/item/UserName/:UserName', function(req,res) {
    if (req.params.UserName) {
        ...
        res.status(500).json({"error": "missing user name"});
    }
})

//Restful insert
app.post('/api/item/UserName/:UserName', function(req,res) {
    if (req.params.UserName) {
        ...
        client.connect(function(err){
            ...
            const db = client.db(dbName);
            let newDocument = {};
            newDocument['UserName'] = req.body.UserName;

   	db.collection('UserName').insertOne(newDocument, function(err,results){
                assert.equal(err,null);
                client.close()
                res.status(200).end()
                    });
...
app.delete('/api/item/UserName/:UserName', function(req,res){
    if (req.params.UserName) {
        ...
        client.connect(function(err){
            ...
            db.collection('UserName').deleteMany(criteria, function(err,results) {
                assert.equal(err,null)
                client.close()
                res.status(200).end();
            })
